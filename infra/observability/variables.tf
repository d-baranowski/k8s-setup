...
variable "create_user" {
  description = "Whether to create IAM users with static access keys and export them to Google Secret Manager"
  type        = bool
  default     = false
}

variable "external_secrets_output_path" {
  description = "Path to write generated ExternalSecret YAML files into (relative paths resolved from this dir)"
  type        = string
  default     = ""
}
